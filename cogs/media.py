    ...
    async def callback(self, interaction: Interaction):
        ...
        if self.current_movie is not None:
            if self.current_movie == movie_id:
                return
        self.movie_index
        embed = Media.create_movie_embed(movie_id=movie_id, index=self.movie_index[f"{movie_id}"], pages=len(self.movie_list))
...
class Media(commands.Cog):
    ...
    def create_media_embed(mediaType: MediaType, results: list, index: int):
        if mediaType == MediaType.movie:
            return Media.create_movie_embed(results[index]['id'], index, len(results))
